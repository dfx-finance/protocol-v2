...
contract CurveFactoryV2Test is Test {
    ...
    // Make sure random people cant make new assimilators
    // function test_FailNotOwnerNewAssimilator() public {
    //     dfxCadcCurve.setAssimilator(
    //         address(usdc),
    //         address(usdcFactoryAssimNew), 
    //         address(usdc), 
    ...
    function testNormalCurve() public {
        cheats.startPrank(address(treasury));
        // Incorrect oracles, with correct token pairing and decimals
        CurveInfo memory badCadcCurveInfo = CurveInfo(
            string.concat("dfx-", cadc.name()),
        ...
        uint256 cadcBalanceBefore = cadc.balanceOf(address(liquidityProvider));
        uint256 usdcBalanceBefore = usdc.balanceOf(address(liquidityProvider));
        console.log(cadcBalanceBefore);
